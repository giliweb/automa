...
class Automa {
    ...
    get pendingCommits(){
        ...
        messages.forEach((e, k) => {
            ...
            if((e.toLowerCase().indexOf('merge branch') === 0 || e.toLowerCase().indexOf('merge remote-tracking branch') === 0) && e.toLowerCase().includes(`into ${this.#masterBranchName}`)){
                return
            }
    ...
    get repoLink(){
        let link = execSync(`git --git-dir=` + gitRoot + ` config --get remote.${this.#remote}.url`).toString().replace('.git', '')
    ...
    get changelogText(){
        if(!this.checkFileExists(`${appRoot}/CHANGELOG.md`)){
            this.createFile(`${appRoot}/CHANGELOG.md`, `# ${this.packageJson.name} Changelog\n\n`)
        }
    ...
    commit(){
        const changelogExists = this.checkFileExists(`${appRoot}/CHANGELOG.md`)
        execSync(`git --git-dir=` + gitRoot + ` add package.json ` + (changelogExists ? 'CHANGELOG.md' : '') + ` && git --git-dir=` + gitRoot + ` commit -m"Update to version ${this.packageVersion} @automa"`)
    }
}
